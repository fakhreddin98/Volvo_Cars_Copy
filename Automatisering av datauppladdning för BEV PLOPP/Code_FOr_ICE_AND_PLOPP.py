...
'''
def move_PLOPP(source_folder, dest_parent_folder):
    total_files = 0
    num_files = 0
...
def move_zip_file(source_item):
    ...
    """
    zip_folder = os.path.join(r"C:\Users\fkabawe\OneDrive - Volvo Cars\Desktop\test")
...
def move_mf4_file(source_item):
    ...
    mf4_folder = os.path.join(r"C:\Users\fkabawe\OneDrive - Volvo Cars\Desktop\test", folder_name_mf4 , 'data')
...
def move_else_file(source_item):
    ...
    """
    else_folder = os.path.join(r"C:\Users\fkabawe\OneDrive - Volvo Cars\Desktop\test", volume_info[0])
    os.makedirs(else_folder, exist_ok=True)
    dest_item = os.path.join(else_folder, os.path.basename(source_item))
    shutil.copy2(source_item, dest_item)


def move_ICE(source_folder, dest_parent_folder):
    total_files = 0
    num_files = 0
    ...
    for root, dirs, files in os.walk(source_folder):
        for item in files:
            ...
            if extension in ('.rar', '.zip'):
                move_zip_ICE(source_item)
                
            elif extension in ('.mf4', '.dat'):
                move_mf4_ICE(source_item)

            else:
                move_else_ICE(source_item)
...
def move_zip_ICE(source_item):
    ...
    Ingen utskrift görs i denna funktion.
    """
    zip_folder = os.path.join(r"C:\Users\fkabawe\OneDrive - Volvo Cars\Desktop\ICE")
    #zip_folder = os.path.join(r"C:\Users\fakhe\Desktop\testing\zip", folder_name_zip)
    os.makedirs(zip_folder, exist_ok=True)
    dest_item = os.path.join(zip_folder, os.path.basename(source_item))
    shutil.copy2(source_item, dest_item)
...
def move_mf4_ICE(source_item):
    """
    samma som förra men till mf4 och dat filerna
    """
    folder_name_mf4 = os.path.splitext(os.path.basename(volume_info[0]))[0].split('_')[0]

    mf4_folder = os.path.join(r"C:\Users\fkabawe\OneDrive - Volvo Cars\Desktop\ICE", folder_name_mf4 , 'data')
    os.makedirs(mf4_folder, exist_ok=True)
    dest_item = os.path.join(mf4_folder, os.path.basename(source_item))
    shutil.copy2(source_item, dest_item)


def move_else_ICE(source_item):
    """
    samma som förra men till alla filer som inte är mf4, dat, zip eller rar
    """
    else_folder = os.path.join(r"C:\Users\fkabawe\OneDrive - Volvo Cars\Desktop\ICE", volume_info[0])
    os.makedirs(else_folder, exist_ok=True)
    dest_item = os.path.join(else_folder, os.path.basename(source_item))
    shutil.copy2(source_item, dest_item)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        #                move_files(drive_folder, source_folder)
                ...
                elif '_VPT_ICE' in volume_info[0]:
                    # Sätter sökvägen till enheten och listar alla filer i sökvägen
                    source_folder = new_drive + ':\\'
                    items = os.listdir(source_folder)
