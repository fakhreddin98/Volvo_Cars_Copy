            ...
            if items:
                ...
                for folder in os.listdir(drive_folder):
                    if folder.startswith(date_str):
                        if folder.startswith(time_str):
                            dest_folder = os.path.join(drive_folder, folder)
                            break

                if not dest_folder:
                    # Skapa en ny mapp för dagens filer
                    dest_folder = os.path.join(drive_folder, f"{volume_info[0]} Datum {date_str} Klocka {time_str}")
