...
delay_lines = [int(delay) for delay in response.text.strip().split('\n')]
...
def add_item():
    ...
    tree.insert('', 0, values=[Url, delay])
...
def clear_all():
    ...
    global urls_list, delay_list
    urls_list, delay_list = [], []
...
def Start():
    ...
    for url, delay in zip(urls_list[1:], delay_list[1:]):
    ...
    while True:
        ...
        try:
            ...
            time.sleep(delay_list[current_tab])
        except:
            time.sleep(delay_list[current_tab])
            continue
